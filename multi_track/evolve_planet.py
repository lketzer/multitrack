...
def evolve_one_planet(pl_folder_pair, 
    ...
    """
    Function evolves one planet (pl) at a time, but through all stellar evolutionary tracks specified in evo_track_dict_list 
    (see below for details). 
    It also makes sure the results are saved in the correct folder-subfolder structure which was previously created 
    with create_planet_chunks.
    All calculations for one planet belong to each other (same host star initial parameters), but the planets themselves 
    are independent of each other. Each time this function is called represents an independent process (important for multiprocessing). 
    
    Parameters:
    -----------
    pl_folder_pair (list): a list with two elements, the first is the folder name, the second the planet object 
    ...
    None
    """
    # TWO Options: (1) track dictionary contains all 9 parameters (e.g. if host star is the same for all planets in sample)
    # or (2) track dictionary contains only 3 parameters (t_sat, dt_drop, Lx_drop_factor) and the remaining parameters have to be
    # tailored to each host star (t_start, t_curr, t_5Gyr, Lx_curr, Lx_5Gyr, Lx_max)
    ...
    if len(evo_track_dict_list[0]) == 9:
        ...
        # evolve the planet through all the complete tracks in evo_track_dict_list
        for track in evo_track_dict_list: 
            pl.set_name(t_final, initial_step_size, epsilon, K_on, beta_on, evo_track_dict=track) # set planet name based on specified track
            ...
            if not os.path.isdir(path_for_saving + pl_file_name):
                pl.evolve_forward_and_create_full_output(t_final, initial_step_size, 
                                                         epsilon, K_on, beta_on, 
                                                         evo_track_dict=track, 
                                                         path_for_saving=path_for_saving, 
                                                         planet_folder_id=folder)
                
                # calculate radius at t_final if planet would undergo ONLY thermal contraction and save to file
                R_thermal = plmoLoFo14.calculate_planet_radius(pl.core_mass, pl.fenv, t_final, pl.flux, pl.metallicity)
    ...
    # Option 2
        ...
        # Need to set Lx_1Gyr & Lx_5Gyr based on the Lx,sat approximation 
        # Scale the parameters at 1 & 5 Gyr up and down for non-solar mass stars, based on the spread in Lx,sat
        # NOTE: in this scenario, t_curr and t_5Gyr are hardcoded to 1 & 5 Gyr!!
        # If you want a track with these values different, you need to pass a complete dictionary

        pl = pl_folder_pair[1]
        folder = pl_folder_pair[0]

        # here I need to select how Lx,sat was calculated -> info stored in pl.Lx_sat_info
        # (this is needed to scale Lx_1Gyr and Lx_5Gyr correctly)
        # 4 options: 
        #     - "OwWu17": use L_XUV as given in Owen & Wu 2017 (NOTE: their XUV values seem to be very low!)
        #     - "OwWu17_X=HE": use Owen & Wu 2017 XUV saturation value as X-ray saturation value
        #     - "Tu15": they use Lx/Lbol = 10^(-3.13)*(L_bol_star)
        #     - "1e-3": approximation of saturation regime (Lx/Lbol ~ 10^(-3))
        
        
        Lx_calculation = pl.Lx_sat_info
        if Lx_calculation == "OwWu17":
            # OwWu 17
            Lx_age_OW17 = undo_what_Lxuv_all_does(L_high_energy(pl.age, pl.mass_star))       
            ...
            same_track_params = {"t_start": pl.age, "t_curr": 1e3, "t_5Gyr": 5e3, 
                                 "Lx_curr": Lx_1Gyr, "Lx_5Gyr": Lx_5Gyr, "Lx_max": pl.Lx_age}

        elif Lx_calculation == "OwWu17_X=HE":
            # OwWu 17, with Lx,sat = L_HE,sat
            Lx_age_OW17_HE = L_high_energy(pl.age, pl.mass_star) #shift based on OwWu17 solar value
            ...
            # need Mass-Luminosity relation to estimate L_bol based on stellar mass
            # NOTE: we ignore any PRE-MS evolution and set L_bol based on MS-values
            ...
            # for the Tu15 tracks we scale the hardcoded values for the SUN at 1 & 5 Gyr up and down based on 
            # a star'ss the difference in Lx,sat as compared to the Sun
            scaling_factor = Lx_sat_Tu15 / (10**(-3.13) * (1.0) * const.L_sun.cgs.value)
            ...
            scaling_factor = Lx_age_1e3 / (10**(-3) * (1.0) * const.L_sun.cgs.value)
            Lx_1Gyr = 2.10*10**28 * scaling_factor  # Lx value at 1 Gyr from Tu et al. (2015) model tracks (scaled)
            Lx_5Gyr = 1.65*10**27 * scaling_factor  # Lx value at 5 Gyr from Tu et al. (2015) model tracks (scaled)
            same_track_params = {"t_start": pl.age, "t_curr": 1e3, "t_5Gyr": 5e3, 
                                 "Lx_curr": Lx_1Gyr, "Lx_5Gyr": Lx_5Gyr, "Lx_max": pl.Lx_age}
        ...
        # and evolve planet through all tracks in evo_track_dict_list
        for t in evo_track_dict_list:
            ...
            if not os.path.isdir(path_for_saving+pl_file_name):
                ...
                R_thermal = plmoLoFo14.calculate_planet_radius(pl.core_mass, pl.fenv, 
                                                               t_final, pl.flux, 
                                                               pl.metallicity)
                filename = folder + "_thermal_contr.txt"
                if not os.path.exists(path_for_saving + filename):
...
def evolve_ensamble(planets_chunks, 
    ...
    for i in range(len(planets_chunks)):
        ...
        # Create a list of processes to run (so all the planets in each chunck/sublist)
        processes = []
        for j in range(len(planets_chunks[i])):
            ...
            # create process and append to list (# instantiating process with arguments)
        ...
        for process in processes:
            ...
            # Make sure Python waits for the process to terminate and then exits the completed processes
            process.join()
...
def evolve_one_planet_along_one_track(folder_planet_track_list, 
                                      t_final, initial_step_size, 
                                      epsilon, K_on, beta_on, 
                                      path_for_saving):
    """
    ...
    Parameters:
    -----------
    folder_planet_track_list (list): a list with three elements, the first is the folder name, the second the planet object, 
                                     the third is a track dictionary (e.g. ["planet1", pl_object1, track_dict1])
                                     NOTE: track_dict is a dictionary with either all 9 parameters per track are specified, 
                                     or a list of track parameters which are different is passed.
                                     This is for a sample where each planet has a different host and the tracks have to be tailored to the 
                                     host star mass/saturation luminosity.
    ...
    # get planet object and folder to save results in, as well as the track dictionary to evolve the star along
    folder = folder_planet_track_list[0]
    planet = folder_planet_track_list[1]
    track_dict = folder_planet_track_list[2]
    
    # Option 1
    if len(track_dict) == 9:
        planet.set_name(t_final, initial_step_size, epsilon, K_on, beta_on, 
                        evo_track_dict=track_dict) # set planet name based on track params
        # generate a useful planet name for saving the results
        pl_file_name = folder + "_track" + planet.planet_id.split("_track")[1] + ".txt"

        # check if result exists, if not evolve planet
        if not os.path.isdir(path_for_saving + pl_file_name):
            planet.evolve_forward_and_create_full_output(t_final, initial_step_size, 
                                                         epsilon, K_on, beta_on, 
    ...
    # Option 2
        ...
        if Lx_calculation == "OwWu17":
            # OwWu 17
            Lx_age_OW17 = undo_what_Lxuv_all_does(L_high_energy(planet.age, planet.mass_star))       
            Lx_1Gyr = undo_what_Lxuv_all_does(L_high_energy(1e3, planet.mass_star))
            Lx_5Gyr = undo_what_Lxuv_all_does(L_high_energy(5e3, planet.mass_star))
            same_track_params = {"t_start": planet.age, "t_curr": 1e3, "t_5Gyr": 5e3, 
                                 "Lx_curr": Lx_1Gyr, "Lx_5Gyr": Lx_5Gyr, "Lx_max": planet.Lx_age}

        elif Lx_calculation == "OwWu17_X=HE":
            # OwWu 17, with Lx,sat = L_HE,sat
            Lx_age_OW17_HE = L_high_energy(planet.age, planet.mass_star) #shift based on OwWu17 solar value
        ...
        # set planet name based on specified track
        planet.set_name(t_final, initial_step_size, epsilon, K_on, beta_on, evo_track_dict=track_dict_complete) 
        # generate a useful planet name for saving the results
        pl_file_name = folder + "_track" + planet.planet_id.split("_track")[1] + ".txt"
