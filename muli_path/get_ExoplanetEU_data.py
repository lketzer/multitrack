...
def get_ExoplanetEU_data():
    ...
    NOTE: returned are only planets which have an orbital period and a radius, as well as a period < 300 days.
...
def gaussian_kernel_density_estimation(X_data, Y_data, baseX, baseY, bandwidth=0.25):
    """
    Function takes 2 dimensional data and constructs a Gaussian kernel density estimate of the distribution
    ...
    Returns:
    --------
    X_grid, Y_grid (array): two arrays which span the grid on which to plot the density distribution (not in log-scale!)
